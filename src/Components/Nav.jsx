...
const Nav = () => {
  ...
  return (
    <div  className="w-full fixed  z-[999]">
      ...
      className="max-w-screen-2xl mx-auto p-5  flex justify-between  items-center lg:p-10">
        <motion.div
          initial={{ opacity: 0 }}
        ...
        >
          <svg
            ...
            className="wordmark-icon w-20 text-white"
            data-v-1932cced=""
          >
            <path
        ...
        <motion.span
          ...
          viewport={{ once: true }}
        >
          <CiMenuFries className="text-white lg:hidden cursor-pointer" />
        </motion.span>
        ...
        <motion.div
          ...
          transition={{ ease: Power4.easeInOut, duration: 0.7 }}
          viewport={{ once: true }}
          className="link hidden gap-5 lg:flex"
        >
          {["Work", "Studio", "News", "Contact"].map((item, index) => (
            <a
              key={index}
              className="text-sm font-semibold text-[#F4F8FA] cursor-pointer lg:text-base"
            >
              {item}
            </a>
          ))}
